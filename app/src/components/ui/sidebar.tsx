...
// One shared geometry for every visual theme. These values are the original
// Default-theme dimensions; theme selection must never resize app chrome.
const SIDEBAR_WIDTH = "15rem"
const SIDEBAR_WIDTH_MOBILE = "min(16.25rem, calc(100vw - 1.25rem))"
  ...
  if (collapsible === "none") {
    return (
      <div
        data-slot="sidebar"
        className={cn(
          "flex h-full w-(--sidebar-width) flex-col bg-sidebar/75 text-sidebar-foreground backdrop-blur-lg backdrop-saturate-150 dark:bg-sidebar/50",
          className
        )}
        {...props}
      >
  ...
  return (
    ...
    >
      ...
      <div
        ...
        className={cn(
          "fixed inset-y-0 z-10 hidden h-svh w-(--sidebar-width) transition-[left,right,width,visibility] duration-200 ease-linear data-[side=left]:left-0 data-[side=left]:group-data-[collapsible=offcanvas]:left-[calc(var(--sidebar-width)*-1)] data-[side=right]:right-0 data-[side=right]:group-data-[collapsible=offcanvas]:right-[calc(var(--sidebar-width)*-1)] group-data-[collapsible=offcanvas]:invisible group-data-[collapsible=offcanvas]:pointer-events-none md:flex",
...
function SidebarInset({ className, ...props }: React.ComponentProps<"main">) {
  return (
    <main
      ...
      className={cn(
        "relative flex min-h-0 min-w-0 w-full flex-1 flex-col bg-background md:bg-transparent md:peer-data-[variant=inset]:m-2 md:peer-data-[variant=inset]:ml-0 md:peer-data-[variant=inset]:rounded-xl md:peer-data-[variant=inset]:shadow-sm md:peer-data-[variant=inset]:peer-data-[state=collapsed]:ml-2",
        className
      )}
      {...props}
    />
