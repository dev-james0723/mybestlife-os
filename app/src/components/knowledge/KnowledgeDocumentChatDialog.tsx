...
/**
 * Generic "Ask the Document" dialog. Used for articles, GitHub READMEs,
 * markdown notes, HTML input, and code cards. Grounded in the item's
 * extracted content via the `/api/knowledge/document-chat` route.
 */
export function KnowledgeDocumentChatDialog({ item, open, onOpenChange, copy }: Props) {
  ...
  const starters = useMemo(() => {
    const fromProvider = [] as string[];
    const out: string[] = [];
    const seen = new Set<string>();
    for (const q of fromProvider) {
      const k = q.toLowerCase().trim();
      if (!k || seen.has(k)) continue;
      seen.add(k);
      out.push(q.trim());
    }
    for (const q of copy.documentChatFallbackQuestions) {
    ...
    return out;
  }, [copy.documentChatFallbackQuestions]);
  ...
  const sendTurn = useCallback(
    async (userText: string) => {
      ...
      setLoading(true);
      try {
        const res = await fetch("/api/knowledge/document-chat", {
          method: "POST",
        ...
        if (!res.ok) {
          ...
          throw new Error(
            friendly ?? data.detail ?? data.error ?? copy.documentChatError,
          );
        }
        const reply = typeof data.reply === "string" ? data.reply : "";
        if (!reply) throw new Error(copy.documentChatError);
        setMessages((m) => [...m, { role: "assistant", content: reply }]);
      } catch (e) {
        const msg = e instanceof Error ? e.message : copy.documentChatError;
        setError(msg);
        ...
        <div className="flex min-h-0 flex-1 flex-col">
          ...
          <div className="shrink-0 border-t p-3 space-y-2">
            <Textarea
              ...
              placeholder={copy.documentChatPlaceholder}
              rows={2}
