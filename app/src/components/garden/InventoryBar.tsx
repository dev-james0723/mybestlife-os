...
export function InventoryBar() {
  ...
  return (
    <OSFrostedPanel className="flex w-fit max-w-full flex-wrap items-center gap-2 px-3 py-2">
      <span className="text-xs font-medium text-muted-foreground">{ui.inventoryLabel}</span>
      {items.map((item) => {
        const cfg = ITEM_CONFIG[item.item_type];
        const Icon = cfg.icon;
        return (
          <Badge key={item.item_type} variant="outline" className="gap-1 border-border/60 bg-background/70 text-xs">
