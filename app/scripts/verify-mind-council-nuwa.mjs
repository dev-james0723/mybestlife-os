...
const outputDir = "../artifacts/mind-council-nuwa-2026-09-08";
const viewports = [
...
try {
  for (const viewport of viewports) {
    ...
    assert(box.y + box.height <= viewport.height + 1, `${viewport.name} dialog extends below the viewport`);
    assert(Math.abs(box.y + box.height / 2 - viewport.height / 2) <= 2, `${viewport.name} dialog is not vertically centered`);
    assert(box.x > 0 && box.y > 0, `${viewport.name} modal is pinned to a viewport edge`);

    assert.equal(await dialog.getByText("AI simulation based on public material — not the real person.", { exact: true }).count(), 1);
    assert.equal(await dialog.getByText("Inspired lens", { exact: true }).count(), 0);
    assert(!(await dialog.getByRole("heading").first().innerText()).includes("Lens"));
    ...
    const sentBox = await dialog.boundingBox();
    assert(sentBox && sentBox.y >= 0 && sentBox.y + sentBox.height <= viewport.height + 1, "Dialog must remain inside the viewport after sending");
    const sendBox = await dialog.getByRole("button", { name: "Send", exact: true }).boundingBox();
    assert(sendBox && sendBox.y + sendBox.height <= viewport.height, "Composer must remain visible");
    const screenshot = `${outputDir}/${viewport.name}.png`;
    const style = await dialog.evaluate(el => ({ inline: el.getAttribute('style'), translate: getComputedStyle(el).translate, transform: getComputedStyle(el).transform, top: getComputedStyle(el).top, height: getComputedStyle(el).height }));
    await page.locator("nextjs-portal").evaluateAll((portals) => {
      for (const portal of portals) portal.style.display = "none";
    });
...
const receipt = {
  ...
  checks: [
    "single AI disclosure and person-name header",
    "first-person response rendering (fixture, not live AI)",
