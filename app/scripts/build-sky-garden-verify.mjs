...
if (!requestedMode) {
  let listening = false;
  try { listening = !!execFileSync("lsof", ["-nP", "-iTCP:3126", "-sTCP:LISTEN"], { encoding: "utf8" }).trim(); } catch { /* No process on the fixture port. */ }
  assert(!listening, "Stop the local preview on port 3126 before rebuilding: a running Next server caches old chunks.");
}
...
process.env.NEXT_OUTPUT_DIR = ".next-sky-verify";
process.env.NEXT_PUBLIC_DEV_LOGIN_BYPASS = "true";
process.env.NEXT_PUBLIC_HIDE_DEV_LOGIN_BYPASS = "false";
process.env.NEXT_PUBLIC_SUPABASE_URL = "http://127.0.0.1:4326";
...
if (requestedMode === "serve" || requestedMode === "dev") {
  ...
  auth.listen(4326, "127.0.0.1"); await once(auth, "listening");
  const server = spawn(process.execPath, ["node_modules/next/dist/bin/next", ...(requestedMode === "dev" ? ["dev", "--webpack"] : ["start"]), "--hostname", "127.0.0.1", "--port", "3126"], { stdio: "inherit" });
...
const manifest = JSON.parse(await readFile(".next-sky-verify/server/app-paths-manifest.json", "utf8"));
