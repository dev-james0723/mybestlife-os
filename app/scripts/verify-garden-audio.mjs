...
try {
  ...
  for (const [name, night, seconds, cues, muted] of [
    ...
    ["garden-sound-effects", false, 30, true, false],
    ...
    const result = await page.evaluate(async ({ night, seconds, cues, muted }) => {
      ...
      } else ["buddy", "plant", "water", "refill", "harvest", "forage", "discover", "dash", "trail-start", "trail-stop", "trail-failed", "trail-won", "deliver", "open", "confirm", "cancel"].forEach((cue, i) => graph.cue(cue, 0.4 + i * 1.75));
