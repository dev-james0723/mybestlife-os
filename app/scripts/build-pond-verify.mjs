...
import { capturePondBuildSources, hashPondSources, verifyPondBuildSources } from "./garden-pond-build-evidence.mjs";
...
if (!requestedMode) {
  let listening = false;
  try { listening = !!execFileSync("lsof", ["-nP", "-iTCP:3122", "-sTCP:LISTEN"], { encoding: "utf8" }).trim(); } catch { /* No process on the fixture port. */ }
  assert(!listening, "Stop the local preview on port 3122 before rebuilding: a running Next server caches old chunks.");
  buildSources = await capturePondBuildSources();
}
...
process.env.NEXT_OUTPUT_DIR = ".next-pond-verify";
process.env.NEXT_PUBLIC_ENABLE_LIVING_POND = "true";
process.env.NEXT_PUBLIC_DEV_LOGIN_BYPASS = "true";
process.env.NEXT_PUBLIC_HIDE_DEV_LOGIN_BYPASS = "false";
process.env.NEXT_PUBLIC_SUPABASE_URL = "http://127.0.0.1:4312";
...
if (requestedMode === "serve") {
  ...
  auth.listen(4312, "127.0.0.1"); await once(auth, "listening");
  const server = spawn(process.execPath, ["node_modules/next/dist/bin/next", "start", "--hostname", "127.0.0.1", "--port", "3122"], { stdio: "inherit" });
  const stop = () => { server.kill("SIGTERM"); auth.close(); };
  process.once("SIGINT", stop); process.once("SIGTERM", stop);
  const [code] = await once(server, "exit"); auth.close(); process.exit(code ?? 0);
}
const routePaths = { app: ["/[locale]/(protected)/garden/page.tsx", "/[locale]/(protected)/dashboard/page.tsx", "/[locale]/(protected)/habits/page.tsx", "/[locale]/(protected)/tasks/page.tsx"], pages: [] };
...
const manifest = JSON.parse(await readFile(".next-pond-verify/server/app-paths-manifest.json", "utf8"));
assert(manifest["/[locale]/(protected)/garden/page"], "Garden route missing from optimized output");
if (!requestedMode) {
  const current = await hashPondSources(Object.keys(buildSources));
  assert.deepEqual(Object.keys(buildSources).filter(path => buildSources[path] !== current[path]), [], "Sources changed during the Garden build; rebuild from a stable revision.");
  await writeFile(".next-pond-verify/garden-source-manifest.json", JSON.stringify({ state: "test-only optimized build", builtAt: new Date().toISOString(), sources: buildSources }, null, 2));
}
