...
export const base = process.env.GARDEN_VERIFY_URL ?? "http://127.0.0.1:3126";
...
export async function createGardenFixture(output) {
  ...
  async function open(viewport, { account = "A", dark = false, touch = false, noWebGL = false, locale = "en", empty = false, reduced = false, record = false, deferNavigation = false, automaticTimezone = false, activityDelayMs = 0 } = {}) {
    ...
    await page.route("**/api/**", route => route.fulfill({ status: 200, contentType: "application/json", body: new URL(route.request().url()).pathname === "/api/garden/pets" ? JSON.stringify({pets:[],available:false,generationAvailable:false}) : "{}" }));
